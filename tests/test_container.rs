...
async fn container_add_task_persists_to_volume() {
    ...
    let add = tool_call(
        ...
        "tok",
        "add_task",
        json!({ "title": "Container task", "autosync": false }),
    )
    .await;
...
async fn container_missing_repo_and_no_git_url_fails() {
    ...
    assert!(
        result.is_err(),
        "expected container to fail without a git URL"
    );
}
...
async fn container_sync_pushes_to_remote() {
    ...
    let add = tool_call(
        ...
        "tok",
        "add_task",
        json!({ "title": "Sync test task", "autosync": false }),
    )
    .await;
    assert!(!is_error(&add), "add_task failed: {add}");
...
async fn container_deferred_sync_fires_after_delay() {
    ...
    for i in 0..3 {
        let add = tool_call(
            ...
            "tok",
            "add_task",
            json!({ "title": format!("Deferred task {i}"), "autosync": false }),
        )
        .await;
