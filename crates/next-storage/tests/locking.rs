...
use next_storage::{GitBackend, TomlStore};
    ...
    let handles: Vec<_> = (0..N)
        .map(|i| {
            ...
            std::thread::spawn(move || {
                ...
                let task_path = next_storage::task_path(&root, &task);
...
fn pull_and_task_save_do_not_trample() {
    ...
    let (mut store_a, vcs_a) = next_storage::open(repo_a.path().to_path_buf()).unwrap();
    let task_a = Task::new("Remote task");
    store_a.save_task(&task_a).unwrap();
    let path_a = next_storage::task_path(repo_a.path(), &task_a);
    ...
    Command::new("git")
        ...
        .status()
        .unwrap();
    next_storage::open(repo_b.path().to_path_buf()).unwrap(); // creates tasks/
