...
use next_storage::{CachedStore, GitBackend};
...
fn clone_of(remote_path: &Path) -> (TempDir, PathBuf, CachedStore, GitBackend) {
    ...
    let (store, vcs) = next_storage::open(clone_path.clone()).unwrap();
...
fn commit_task(
    ...
    let path = next_storage::task_path(repo_root, &task);
...
fn fresh_store(root: &Path) -> CachedStore {
    next_storage::open(root.to_path_buf()).unwrap().0
}
...
fn push_without_remote_errors() {
    ...
    run_git(dir.path(), &["config", "user.email", "test@example.com"]);
    run_git(dir.path(), &["config", "user.name", "Test"]);
    let (mut store, vcs) = next_storage::open(dir.path().to_path_buf()).unwrap();
...
fn push_then_pull_preserves_all_task_data() {
    ...
    a_store.save_task(&task).unwrap();
    let path = next_storage::task_path(a_dir.path(), &task);
