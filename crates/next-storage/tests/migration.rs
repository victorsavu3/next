...
/// table), then calls `next_storage::open()` to simulate a fresh process opening an
...
fn open(dir: &TempDir) -> CachedStore {
    let (store, _vcs) = next_storage::open(dir.path().to_path_buf()).unwrap();
    store
}
