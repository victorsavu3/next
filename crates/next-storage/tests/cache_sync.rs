...
use next_storage::{CachedStore, GitBackend, TomlStore};
...
fn same_head_reuses_cache_does_not_see_bypass_writes() {
    ...
    let known_path = next_storage::task_path(dir.path(), &known);
...
fn pull_adds_new_task() {
    ...
    let path = next_storage::task_path(dir.path(), &existing);
    ...
    let pulled_path = next_storage::task_path(dir.path(), &pulled);
...
fn pull_removes_task() {
    ...
    let keep_path = next_storage::task_path(dir.path(), &keep);
    let remove_path = next_storage::task_path(dir.path(), &remove);
...
fn pull_adds_multiple_tasks() {
    ...
    let seed_path = next_storage::task_path(dir.path(), &seed);
    ...
    for t in &new_tasks {
        ...
        paths.push(next_storage::task_path(dir.path(), t));
    }
...
fn pull_removes_multiple_tasks() {
    ...
    for t in removed {
        let p = next_storage::task_path(dir.path(), t);
...
fn pull_mixed_changes() {
    ...
    let path_a = next_storage::task_path(dir.path(), &existing_a);
    let path_b = next_storage::task_path(dir.path(), &existing_b);
    let path_c = next_storage::task_path(dir.path(), &existing_c);
    ...
    let path_d = next_storage::task_path(dir.path(), &task_d);
...
fn pull_updates_state() {
    ...
    store.save_task(&task).unwrap();
    let task_path = next_storage::task_path(dir.path(), &task);
