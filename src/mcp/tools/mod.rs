        ...
        Tool {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "fields": { "type": "array", "items": { "type": "string" }, "description": "Return only these fields, e.g. [\"id\",\"title\",\"due\"]. Names are the filter grammar's: id, title, status, priority, due, start, completed, slug, parent, assignee, tags, blocked_by, description, notes, url, recurrence, long_term, score_adjustment, data, data.<key>, score, and score_breakdown (that last one only ever has a value on get_task). `created`/`updated` are filterable but not projectable — they come from git history, not from the task. A field not asked for is ABSENT from the object rather than null. Note that score and score_breakdown are NOT task fields: asking for score alone gives you {\"score\":…,\"task\":{}} — you probably want [\"id\",\"score\"]. Worth using: the default returns every field of every task, and notes can run to thousands of words." },
        ...
        Tool {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "fields": { "type": "array", "items": { "type": "string" }, "description": "Return only these fields on the task and its children, e.g. [\"id\",\"title\",\"due\"]. Same names as list_tasks, plus score and score_breakdown, which sit beside the task rather than in it. With no fields you get everything; name any field and score and score_breakdown are dropped unless you name them too — the breakdown is the largest object in the response. A field not asked for is absent rather than null." }
                }
            }),
        },
        Tool {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "recur_snap_leeway": { "type": "string", "description": "Tolerance around the snap boundary, in days: \"3\" for both directions, \"BACK,FORWARD\" e.g. \"5,0\", or \"BACK,*\" e.g. \"5,*\" to leave the forward direction unbounded. The raw date moves to a boundary only if one falls within the window; otherwise it is kept as is, preserving the interval. Requires recur_snap. Omit to keep the default, which never pulls a date earlier and always pushes it later — that default lengthens the cycle whenever a task is completed late." },
